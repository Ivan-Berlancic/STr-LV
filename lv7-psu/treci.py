...
while True:
    ...
    for rect in rects:
        ...
        cropped_digit = np.expand_dims(cropped_digit, axis=0) # add batch dimension
        cropped_digit = np.expand_dims(cropped_digit, axis=-1) # add channel dimension
        label = model.predict(cropped_digit)
        ...
        cv2.putText(frame, str(label), (rect[0], rect[1]), font,
            fontScale = 0.5,
            color = (255, 0, 0),
            thickness =  1,
